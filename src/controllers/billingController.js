...
// Create new bill (supports manual/custom products)
export const createBill = async (req, res) => {
  try {
    ...
    for (const item of items) {
      if (item.isCustom) {
        // Use manual/custom item directly
        billItems.push({
          name: item.name,
          size: item.size,
          mrp: item.mrp,
    ...
    // Calculate totals
    const subtotal = billItems.reduce((acc, i) => acc + i.mrp * i.quantity, 0);
    const totalRCP = billItems.reduce((acc, i) => acc + i.rcp * i.quantity, 0);
    ...
    return res.status(201).json({ success: true, bill: savedBill });

  } catch (error) {
    console.error('CREATE BILL ERROR:', error);
    return res.status(500).json({ success: false, message: 'Server error while creating bill', error: error.message });
  }
};
...
export const generateBillPDF = async (req, res) => {
  try {
    const { id } = req.params;
    const bill = await Bill.findById(id).populate('items.productId', 'name size');
    ...
    bill.items.forEach(item => {
      ...
      const discountPerItem = (item.mrp * item.quantity) - item.total;
      doc.text(`MRP: ₹${item.mrp} (Save: ₹${discountPerItem.toFixed(2)})`, 10, y);
      doc.fontSize(8).font('Helvetica');
      y += 10;
    });
    ...
    // QR code
    const qrData = { billNumber: bill.billNumber, date: bill.date, total: bill.total };
...
export const getSalesSummary = async (req, res) => {
  try {
    ...
    const bills = await Bill.find({ date: { $gte: startDate, $lte: endDate } });
    const summary = {
      totalBills: bills.length,
      totalSales: bills.reduce((sum,bill) => sum + bill.total,0),
      totalDiscount: bills.reduce((sum,bill) => sum + bill.discount,0),
      totalItems: bills.reduce((sum,bill) => sum + bill.items.reduce((iSum,item)=>iSum+item.quantity,0),0),
      averageBillValue: bills.length>0 ? bills.reduce((sum,bill)=>sum+bill.total,0)/bills.length : 0
    };
    res.json({ success:true, period, startDate, endDate, summary });
  } catch(error) {
